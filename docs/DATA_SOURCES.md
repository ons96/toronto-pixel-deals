...
|---|---|---|---|
| `data/static_specs.json` | Bundled offline baseline; seeded by the private deployment fixture setup | Hardware-spec fields, `source=static`, local fetch timestamp | Original provenance, version, and license are not yet recorded; not eligible for public/commercial refresh. |
...
`scripts/update_site.sh` and `/deals/refresh` fail closed. Re-enable neither
until the applicable source reviews are documented.
