# RapidAPI Listing Copy: Toronto Pixel Deal Score API
...
against the same cached listings, no re-fetch required. The data is refreshed from
Kimovil (hardware specs), Reddit, eBay, and Frankfurter (USD-to-CAD FX), and a
Pro-tier endpoint triggers a fresh fetch on demand.
...
|---|---|---|---|---|
| GET | `/health` | none | -- | `{"status":"ok","db":{"specs":22,"listings":3,"fx":0},"version":"1.0"}` |
| GET | `/deals/top` | counted | `n` (1-500), `cpu`,`battery`,`camera`,`display`,`form_factor` (0-1), `include_unknown` (bool) | `{"weights":{...},"count":3,"deals":[{...}],"tier":"free","used":1,"limit":100}` |
| GET | `/deals/refresh` | counted + Pro+ | -- | `{"status":"refreshed","db":{...}}` or `403 tier_required` |
...
# or BASE = "http://your-server:8100" for direct/customers
...
PixelDealScore is the only place that joins hardware specs (Kimovil) onto live
used listings (Reddit/eBay) and ranks them on a single, re-weightable
...
**Low-cost, reliable infra.** Runs on Oracle Cloud's free tier with SQLite as the
single store -- no database bill to pass through, which keeps the pricing low.
